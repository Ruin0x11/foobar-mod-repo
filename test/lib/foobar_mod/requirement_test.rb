...
class FoobarMod::TestRequirement < ActiveSupport::TestCase
  def test_concat
    r = req ">= 1"
  ...
  def test_as_list
    r = req ["< 1.0.0", ">= 0.0.0"]
    assert_equal r.as_list, ["< 1.0.0", ">= 0.0.0"]
  end
  ...
  def test_create
    assert_equal req("== 1"), FoobarMod::Requirement.create("== 1")
    assert_equal req(">= 1.2", "<= 1.3"), FoobarMod::Requirement.create([">= 1.2", "<= 1.3"])
    assert_equal req(">= 1.2", "<= 1.3"), FoobarMod::Requirement.create(">= 1.2", "<= 1.3")
  end

  def test_empty_requirements_is_none
    r = FoobarMod::Requirement.new
    assert_equal true, r.none?
  end

  def test_explicit_default_is_none
    r = FoobarMod::Requirement.new ">= 0"
    assert_equal true, r.none?
  end

  def test_basic_non_none
    r = FoobarMod::Requirement.new "== 1"
    assert_equal false, r.none?
  end

  def test_parse
    assert_equal ["==", FoobarMod::Version.new(1)], FoobarMod::Requirement.parse("== 1")
    assert_equal ["==", FoobarMod::Version.new(1)], FoobarMod::Requirement.parse("   1")
    assert_equal [">", FoobarMod::Version.new(1)], FoobarMod::Requirement.parse("> 1")
    assert_equal ["==", FoobarMod::Version.new(1)], FoobarMod::Requirement.parse("1.0")
    assert_equal [">=", FoobarMod::Version.new(0)], FoobarMod::Requirement.parse("*")

    assert_equal ["==", FoobarMod::Version.new(2)],
      FoobarMod::Requirement.parse(FoobarMod::Version.new("2"))
  end
  ...
  def test_parse_bad
    [
      ...
      e = assert_raises FoobarMod::Requirement::BadRequirementError do
        FoobarMod::Requirement.parse bad
      end

      assert_equal "Illformed requirement [#{bad.inspect}]", e.message
    end

    assert_equal FoobarMod::Requirement::BadRequirementError.superclass, ArgumentError
  end
  ...
  def test_satisfied_by_eh_good
    ...
    assert_satisfied_by "9.3.2 ",      ">= 9.3.2"
    assert_satisfied_by "0.0",         "*"
  end

  def test_illformed_requirements
    [ ">>> 1.3.5", "> blah" ].each do |rq|
      assert_raises FoobarMod::Requirement::BadRequirementError, "req [#{rq}] should fail" do
        FoobarMod::Requirement.new rq
      end
    end
  end
  ...
  def test_satisfied_by_eh_non_versions
    ...
    assert_raises ArgumentError do
      req(">= 0").satisfied_by? FoobarMod::Requirement.default
    end
  end
  ...
  # Assert that two requirements are equal. Handles
  # FoobarMod::Requirements, strings, arrays, numbers, and versions.
